 ...
 const colorM = d3.scaleSqrt().range(['lightyellow','red']);
 ...
 // Loading data
 d3.csv("UE_migration_fix.csv", function(row) {
     ...
     colorM.domain([0,d3.max(edges, d => d.value)]);
 ...
 function draw(nodes, data) {
     ...
     const svg = d3.select("#migration").append("svg").attr("width",width).attr("height",height);
     ...
     cell.append("rect")
             ...
             .style("stroke", 'black')
             .style("fill", d => d.value ? colorM(d.value) : 'white')
 ...
 function highlight(d) {
     ...
     d3.select('.tooltip').select("text")
             .attr("x", d.w * .75)
             .attr("y", d.h * .75)
             .text(d.value ? formatM(d.value/1000) : 0); // Some of the values are in hundred thousands and it doesn't fit in cell
     ...
     d3.select('.tooltip')
             ...
             .attr("rx", d.w/4)
             .attr("ry", d.h/4)
             .style("fill", d.value ? colorM(d.value) : 'white')
             .attr("width", 1.5 * d.w)
             .attr("height", 1.5 * d.w)
 } */
// SVG element
...
d3.csv("EU_migration_fix_newFixed.csv", function (row) {
        ...
        drawMigration(filteredNodes, result);
});

// Putting it all together
function drawMigration(nodes, data) {
        ...
        const tooltip = chart
                .append("g")
                .attr("class", 'tooltip-mig tooltip-class')
                .style("opacity", 0)
                .style("pointer-events", 'none');
...
function unHighlight(d, i, nodes) {
        d3.selectAll(".cell, text.source, text.target").classed('faded highlight', false);
        d3.select('.tooltip-mig').style("opacity", 0);
}
...
function highlight(d) {
        ...
        d3.select('.tooltip-mig').select("text")
                .attr("x", d.w * .75)
                .attr("y", d.h * .75)
                .text(d.value ? formatM(d.value / 1000) : 0);

        d3.select('.tooltip-mig')
                .attr("transform", `translate(${[d.x - d.w / 4, d.y - d.h / 4]})`)
                .style("opacity", 1)
                .select("rect")
                .style("stroke-width", 3)
